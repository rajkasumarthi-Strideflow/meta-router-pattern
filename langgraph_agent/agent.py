...
def call_tool_node(state: AgentState) -> AgentState:
    """
    Node 3: Execute the tool Claude selected.
    Enterprise analog: tool dispatch — calling the governed MCP tool contract.
    Results are fed back into the reasoning loop as ToolMessages.
    """
    step     = state["step"]
    tools    = STEP_TOOLS.get(step, [])
    ...
    for tc in last_msg.tool_calls:
        ...
        if fn is None:
            ...
            try:
                ...
                # Track confirmed bookings for saga state propagation
                if tc["name"] in ("confirm_flight", "confirm_hotel") and parsed.get("success"):
                    result_dict = parsed.get("booking", {})
...
def route_after_tool(state: AgentState) -> Literal["reason", END]:
    """Route: after a tool call, if a booking was confirmed, end; else keep reasoning."""
    result = state.get("result", {})
    # If we already have a confirmed booking in result, we're done
